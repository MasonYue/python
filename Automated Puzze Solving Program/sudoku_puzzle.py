...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self):
        ...
        """
        Return True iff Sudoku Puzzle self can never be extended to a solution.
        ...
        for i in symbols:
            ...
            # if not *,check next one
                ...
                if len(allowed_symbols) == 0:
                    # if no allowed symbols, then return True
                    return True
                else:
                    # else index + 1
                    index += 1
        # if all of missing place can have a solution, then return False
        return False
