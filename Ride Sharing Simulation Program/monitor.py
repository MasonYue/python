...
class Monitor:
    ...
    def notify(self, timestamp, category, description, identifier, location):
        ...
        #if this guy never registered, then register it
    ...
    def _average_total_distance(self):
        ...
        can be cancelled ? calculate cancel or not seperately
        ...
        for activities in self._activities[DRIVER].values():
            #for every driver has more than two activities, first must be Request,
            #then Pickup or Cancell, then Dropoff. We calculate every distance
            if len(activities) >= 2:
                for i in range(len(activities)-1):
                    distance += activities[i].location.manhattan_distance(activities[i+1].location)
                count += 1
    ...
    def _average_ride_distance(self):
        ...
        for activities in self._activities[DRIVER].values():
            #The first activity is REQUEST, and the second is PICKUP
            # and the third one is DROPOFF.
            # The ride distance is the difference between
            # PICKUP location and DROPOFF location
            if len(activities) > 2:
                total_distance += activities[1].location.manhattan_distance(activities[2].location)
                count += 1
